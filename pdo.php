...
if (isset($_POST['submit_parcel'])){
  ...
  $pdoresult1 = $handle->prepare($pdoquery1);

  $pdoexec1 = $pdoresult1->execute(array(":code"=>$Code,":fname"=>$fName,":mname"=>$mName,":lname"=>$lName,":contact"=>$cOntact,":address"=>$aDdress,":FNAME"=>$FName,":MNAME"=>$MName,":LNAME"=>$LName,":CONTACT"=>$COntact,":DESTINATION"=>$DEstination,":type"=>$tYpe,":size"=>$sIze,":weight"=>$wEight,":class"=>$cLass,":delivery"=>$dElivery,":pick"=>$pIck,":deyt"=>$dAte,":taym"=>$tIme,":status"=>$sTatus));
