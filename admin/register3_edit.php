...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <h6 class="m-0 font-weight-bold text-primary">Edit Parcels 
    </h6>
  </div>
  ...
  <?php
    ...
    $connection = mysqli_connect("localhost","root","Ilovenature07");
    $db = mysqli_select_db($connection,'delivery_service_db');

  if(isset($_POST['edit_btn'])){
    $id = $_POST['edit_id'];

    $query = "SELECT * FROM parcels WHERE Parcel_id = $id ";
   ...
   <form id="form" action="" method="POST">  
    <div class="table-responsive">
            <input type="hidden" id="edit_id" name="edit_id" value="<?php echo $row['Parcel_id']; ?>">
            <div class="form-group">
                <input type="hidden" id="code" name="code" class="form-control" placeholder="Enter your First Name" value="<?php echo $row['Reference_number']; ?>">
            </div>
            <div class="form-group">
             ...
             <div class="form-group">
                ...
                default:
                  ...
                  break;
                    ...
                    <option value="10"> Item Accepted by Courier </option>
                    <option value="0"> Pending </option>
                </select>
            </div>
          <div class="modal-footer">
            <a href="register3.php" class="btn btn-danger">CLOSE</a>
            <input type="submit" id="updatebtn" name="update_btn" class="btn btn-primary" value="Update"></input>
        </div> 
    </div>
    </form>  
              <?php
